...
public static class IDatabaseContextExtensions
{
	...
	public static async Task<TModel> GetSingleAsync<TProperty, TModel>(this DbSet<TModel> dbSet, Expression<Func<TModel, bool>> predicate, Expression<Func<TModel, TProperty>> orderBy, bool desc = false, params Expression<Func<TModel, object>>[] includeProperties) where TModel : class
	{
		if (includeProperties == null)
			return await (desc
				? dbSet.OrderByDescending(orderBy).FirstOrDefaultAsync(predicate)
				: dbSet.OrderBy(orderBy).FirstOrDefaultAsync(predicate));

		var query = includeProperties.Aggregate(dbSet.AsQueryable(), (current, property) => current.IncludeConvert(property));

		return await (desc
			? query.OrderByDescending(orderBy).FirstOrDefaultAsync(predicate)
			: query.OrderBy(orderBy).FirstOrDefaultAsync());
	}
	...
	// Context Extensions
	public static void Create<TModel>(this IDatabaseContextBase db, TModel model) where TModel : class
	{
